...
function get_receipt($conn, $bphone, $date, $receipt)
{
  $receipt .= '<div class="row" style="width: 40%;">
        <h4 style="display: inline-block;">Buyer Name: </h4><span>Muhammad abubakar</span>
        <h4 style="display: inline-block;">Phone Number:</h4><span>08169895827</span>
...
{
  $sql = "SELECT *, SUM(sales.qty*sales.price) AS total FROM sales LEFT JOIN receipts ON sales.id=receipts.saleId WHERE receipts.buyerPhone='141' AND receipts.date='2022-06-13'";
  // $sql = "SELECT s.id,s.qty,s.price,r.saleId,r.buyerPhone,r.date,p.name FROM sales s LEFT JOIN receipts r ON r.saleId=s.id RIGHT JOIN products p ON p.id=s.product_id WHERE r.date='2022-06-13' AND r.buyerPhone='141'";
  $result = $conn->query($sql);
  if ($result) {
    $num_rows = $result->num_rows;
...
get_receipt($mysqli, "141", "2022-06-13", $receipt);

get_total($mysqli, "141", "2022-06-13", $receipt);
}
